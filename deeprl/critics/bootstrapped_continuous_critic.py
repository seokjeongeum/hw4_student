...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def compute_target_value(self, next_obs, rewards, terminals, actor):
        ...
        """
        target_value = rewards + self.gamma * self.critic_network(
            torch.cat((next_obs, actor(next_obs).sample()), dim=1)
        ).squeeze() * (1 - terminals)
        """
        END CODE
        """
    ...
    def update_target_network_ema(self):
        ...
        """
        for t, c in zip(
            self.target_network.parameters(), self.critic_network.parameters()
        ):
            alpha = self.target_update_rate
            t.data = (1 - alpha) * t.data + alpha * c.data
        """
        END CODE
        """
