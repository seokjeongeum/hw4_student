...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        """
        list_of_discounted_returns = np.full_like(
            rewards, (self.gamma ** np.arange(len(rewards))) @ rewards
        )
        """
        END CODE
        """
    ...
    def _discounted_cumsum(self, rewards):
        ...
        """
        discount_factors = self.gamma ** np.arange(len(rewards))
        list_of_discounted_cumsums = (
            np.flip(np.cumsum(np.flip(discount_factors * rewards))) / discount_factors
        )
        """
        END CODE
        """
